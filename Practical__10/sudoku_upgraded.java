...
class SudokuGame_upgraded{
    public int gridSize;
    public int[][] board;
    public String[][] displayBoard;

    public SudokuGame_upgraded(int size, int numLength) {
        this.gridSize = size;
        ...
        System.out.println(drawGrid(gridSize, numLength));
    ...
    public String[][] makePuzzle(int size, int length) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                if (rand == i || rand == j) {
                    displayBoard[i][j] = String.format("%" + length + "s", board[i][j]);
                } else {
                    displayBoard[i][j] = String.format("%" + length + "s", "");
                }
            }
        }
        return displayBoard;
    ...
    // Let user enter the missing values
    public void takeInputFromUser(int size, int length) {
        Scanner sc = new Scanner(System.in);

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (displayBoard[i][j].isBlank()) {
                    System.out.print("Enter value at [" + (i + 1) + "," + (j + 1) + "]: ");
                    displayBoard[i][j] = Integer.toString(sc.nextInt());
                }
            }
        }

        if (checkSolution()) {
            System.out.println("Nice! You solved it correctly.");
        } else {
            System.out.println("Oops! Something's wrong with your solution.");
        }
    }
    ...
    public static void main(String[] args) {
        ...
        SudokuGame game = new SudokuGame(size, length);
    }
}
